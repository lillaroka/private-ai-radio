...
export function trimLeadingSilence(buffer, trimMs) {
  const parsed = parseWav(buffer);
  const { data, audioFormat, channels, sampleRate, bitsPerSample } = parsed;
  const trimSamples = Math.round((trimMs / 1000) * sampleRate) * channels * (bitsPerSample / 8);
  if (trimSamples <= 0 || trimSamples >= data.length) return buffer;
  return wavFromPcm({
    pcm: data.subarray(trimSamples),
    audioFormat,
    channels,
    sampleRate,
    bitsPerSample,
